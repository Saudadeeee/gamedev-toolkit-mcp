...
All seven MCP servers live under [`servers/`](../servers/) and are tracked in git.
...
|---|---|---|
| `first-party` | `aseprite`, `godot-mcp`, `rfxgen`, `ffmpeg` | this repo |
...
5. Writes `mcp_config.json` covering **all seven** servers, with this machine's
...
| `ffmpeg` | **No** — spawns ffmpeg per call | `FFMPEG_PATH` if not auto-detected |
