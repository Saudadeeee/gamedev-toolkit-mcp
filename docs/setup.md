...
|---|---|---|
| [Python](https://python.org) | 3.12+ | Runs the `aseprite` server |
...
2. Installs the `aseprite` server's dependencies.
3. Builds `servers/godot/server/dist/index.js`.
...
2. Start from `mcp_config.json`, which `setup.sh` / `setup.ps1` writes with
...
```json
{
  "mcpServers": {
    "aseprite": {
      ...
      "args": [
        "--directory",
        "/absolute/path/to/servers/aseprite",
        "run",
        "-m",
        "aseprite_mcp"
      ],
      "cwd": "/absolute/path/to/servers/aseprite",
      "env": {
    ...
    "godot-mcp": {
      "command": "node",
      "args": [
        "/absolute/path/to/servers/godot/server/dist/index.js"
      ],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
...
```bash
claude mcp add aseprite uv -- --directory /absolute/path/to/servers/aseprite run -m aseprite_mcp
claude mcp add godot-mcp node -- /absolute/path/to/servers/godot/server/dist/index.js
```
...
### Test `servers/godot/server`

```bash
node servers/godot/server/dist/index.js
```
...
- Run `npm run build` inside `servers/godot/server`.
